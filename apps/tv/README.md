...
### Вход: код сопряжения, не пароль

**Пароль в сборке больше не нужен.** Вход — сопряжение по коду (CONTRACT §11): при первом
запуске сервер уводит WebView на страницу `/pair`, она показывает код вида `WDJB-MJHT`,
человек одобряет его с телефона, телевизор получает сессию в куке и дальше работает по ней.

Kotlin для этого не рисует ничего: экран сопряжения — обычная веб-страница. От приложения
требуется другое — **хранить куку**:

- `CookieManager.setAcceptCookie(true)` включается явно. Умолчание зависит от прошивки и
  версии System WebView, и полагаться на него нельзя: ошибка проявится не сразу, а через
  сутки, на первом перезапуске — кодом сопряжения вместо дневника;
- `CookieManager.flush()` вызывается **при паузе и после каждой успешной загрузки**. Куки
  живут в памяти и попадают на диск лениво, а телевизор выключают из розетки — `onPause`
  может не случиться вовсе.

Это и есть главный выигрыш: раньше пароль лежал в `BuildConfig`, то есть в dex, и
доставался из APK одной командой — **только поэтому репозиторий обязан был быть приватным**.

#### Старые параметры basic auth

`-PDASHBOARD_USER` и `-PDASHBOARD_PASSWORD` **устарели** и оставлены ровно до того момента,
пока боевой сервер ещё за Caddy: APK из этой ветки должен работать и со старым сервером,
и с новым. После снятия basic auth собирайте без них:

```bash
./gradlew assembleDebug -PDASHBOARD_URL=https://bt.nuanu.ai
```

Проверить, что попадёт в APK:

```bash
./gradlew :app:printDashboardUrl -q
# DASHBOARD_USER      = (не задан — так и надо, вход по коду)
# DASHBOARD_PASSWORD  = (не задан — так и надо)
```

Пока параметры заданы, работают прежние правила: пара уходит только на хост из
`DASHBOARD_URL` и только по HTTPS, не чаще одной серии попыток, и не попадает ни в logcat,
ни в лог консоли страницы, ни на экран.
...
| Экран «СЕРВЕР НЕ ПРИНЯЛ ЛОГИН И ПАРОЛЬ» | Только для старого сервера за Caddy: `DASHBOARD_USER`/`DASHBOARD_PASSWORD` не совпали — пересобрать с верными |
| «Сервер требует логин и пароль (401), а в сборке их нет» | Старый сервер за Caddy, а APK собран без `-PDASHBOARD_USER/-PDASHBOARD_PASSWORD` |
...
| Код сопряжения каждый раз при включении | Куки не сохраняются. Проверить `adb logcat \| grep -i cookie`: `CookieManager недоступен` означает отсутствующий или обновляющийся System WebView |
| Код сопряжения вместо дневника посреди работы | Устройство отозвали из админки. Одобрите заново: Настройки → Устройства на телефоне |
