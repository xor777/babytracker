...
const UNKNOWN_DEVICE_TEXT =
  'Этот голос пока не подключён к дневнику. ' +
  'В дневнике на телефоне откройте Настройки, «Одобрить и отозвать», ' +
  'нажмите «Подключить новый голос» и повторите фразу.';
...
function logUnknownIdentity(ctx: AppContext, access: AccessCheck, ip: string): void {
  ...
  ctx.log.warn(
    ...
    `alice: незнакомое обращение (${describeIdentity(access.identity)}). ` +
      'Владелец НЕ заблокирован навсегда: в админке на экране устройств ' +
      '«Подключить новый голос», затем повторить фразу.',
  );
}
