...
/**
 * Таксономия одной строкой на тип. Источник правды тот же (`TAXONOMY`), но
 * рендер плотнее, чем `taxonomyForPrompt()`: в промпте это справочник, а не
 * документация, и каждая лишняя строка здесь отнимает внимание у политики.
 */
...
- ended_at = null означает «событие ИДЁТ ПРЯМО СЕЙЧАС». Точечные события
  (diaper, note, measure, meds, symptom и кормление, которое уже закончилось)
  создавай с ended_at = started_at, иначе они навсегда останутся «идущими».
  Пустым ended_at оставляй только начавшийся сон и то, что длится в эту минуту.
