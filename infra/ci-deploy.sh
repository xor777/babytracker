...
#
# ВАЖНО: весь скрипт обёрнут в { ... } намеренно, не убирай фигурные скобки.
# Скрипт обновляет сам себя (git reset затрагивает и этот файл), а bash читает
# файл кусками по мере исполнения — подменив файл на середине, мы бы продолжили
# со старого смещения и выполнили мешанину из двух версий. Блок заставляет bash
# разобрать файл целиком до первой команды, а `exit` в конце гарантирует, что
# за пределы блока чтение не уйдёт.
{
  set -euo pipefail
